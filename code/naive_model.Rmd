...
res_corr <- res |> mutate(nxt_temp = lag(temp), nxt_.pred = lag(.pred))

res_corr |> group_by(hour) |> summarize(cor(temp, nxt_temp, use = "complete.obs"))
res_corr |> group_by(hour) |> summarize(cor(.pred, nxt_.pred, use = "complete.obs"))
