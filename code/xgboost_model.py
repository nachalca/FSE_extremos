...
class XgboostDownscaler():
    ...
    def predict(self, data, model, variable_name):
    ...
    def optimize(self, X_train, y_train, variable_name, **space):
    ...
    def fit(self):
        ...
        for f in files:
            if f.endswith('.csv'):
                ...
                trials = Trials()
                best = fmin(
                        fn=lambda space: self.optimize(X_train=X_train, y_train=y_train, variable_name=variable_name, **space),            
                        space=hyper_params,           
                        algo=tpe.suggest,            
                        max_evals=5,            
                        trials=trials,
