...
lstm_predicted <- fread('lstm_output.csv') 

res <- res |> slice(6:nrow(res)) |> cbind(lstm_predicted) |> rename("lstm_sfcWind" = "V1") |> 
...
grid.arrange(p1, p2, p3, p4,p5, nrow = 5, top = "How Often Peaks Hit Hourly")
```
...
res_to_plot <- data.frame(model = c(rep("obs", 48), rep("nv", 48), rep("knnreg", 48), rep("xgboost", 48),
                                    ...
                                    rep("lstm",48)),
                          lag = c(1:48, 1:48, 1:48, 1:48, 1:48, 1:48, 1:48, 1:48),
                          extremogram = c(p1,p2,p3,p4,p5,p6,p7,p8))
...
res_to_plot <- data.frame(model = c(rep("obs", 48), rep("nv", 48), rep("knnreg", 48), rep("xgboost", 48),
                                    rep("nv_reduced", 48), rep("knnreg_reduced", 48), rep("xgboost_reduced", 48), rep("lstm", 48)),
                          lag = c(1:48, 1:48, 1:48, 1:48, 1:48, 1:48, 1:48, 1:48),
                          acf = c(p1$acf,p2$acf,p3$acf,p4$acf,p5$acf, p6$acf, p7$acf, p8$acf))
