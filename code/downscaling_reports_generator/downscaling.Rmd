...
for(f in to_be_downscaled){
  ...
  res[[paste0("original_", lab, '_', scenario)]] <- variable[[params$variable]] 
  res[[paste0("xgboost_", lab, '_', scenario)]]  <- xgboost_predictions[["xgboost"]]
  res[[paste0("cnn_", lab, '_', scenario)]]  <- cnn_predictions[["cnn"]][1:length(variable[[params$variable]] )] #TODO: CHECK DIMENSIONS
  res[[paste0("nv_", lab, '_', scenario)]] <- nv_predictions[["naive"]]
  #rm(variable)
}
...
```{r metrics}
models <- res_validation |> select(-c("time", "reanalysis", starts_with('original_'))) |> colnames()
...
r <- lapply(models, function(x) {
    if (conf[["VARIABLES"]][[params$variable]][["daily"]]) {
      ...
      metrics_daily(time = res_validation$time, truth = res_validation$reanalysis, estimate = res_validation[[x]], model = x)
    }
})
...
series_plot <- function(data, col) {
  ...
  original <- data[[paste0('original_', unlist(str_split(col,'_'))[2], '_', unlist(str_split(col,'_'))[3])]]
  ...
  ggplot(p, aes(x=time)) +
    ...
    geom_line(aes(y=original), color = "red", linetype="dotted") +
...
```{r acf}
models <- res_validation |> select(-c("time",  starts_with('original_'))) |> colnames()
r <- lapply(models, function(x) {
