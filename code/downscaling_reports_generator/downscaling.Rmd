...
for(f in to_be_downscaled){
  ...
  res[[paste0("cnn.", lab, '-', scenario)]]  <- cnn_predictions[["cnn"]][1:length(variable[[params$variable]] )] #TODO: CHECK DIMENSIONS
...
```{r qqplot, fig.height=9, fig.width=9}
...
extremes <- function(data, exp) {
 res_to_plot <- data |> 
  ...
  mean_on_days_with_extremes_plot(res_to_plot, standarize = TRUE) +
    ggtitle(paste(exp))
}
