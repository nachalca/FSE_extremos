...
- Validation Period:  `r min(res$time)` to `r max(res$time)`

- Response variable to predict:   upscaled ERA5 (daily or monthly)

- Predictors variables: daily scale the variables `sfcWind`, `tas`, `pr`, `tasmax`, `tasmin` and `psl` and in a monthly scale `clt`, `rsdt`,`rsds` ,  the month `r  if (conf[["VARIABLES"]][[params$variable]][["daily"]]) {", hour, sun's elevation & azimuth"}` and the daily daylight amount in seconds.
